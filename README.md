...
This repository does not ship its own container. You can build and use the container from the
[aos_core_lib_cpp](https://github.com/aosedge/aos_core_lib_cpp) repo — its image already contains every build
dependency listed above. See
[this chapter](https://github.com/aosedge/aos_core_lib_cpp?tab=readme-ov-file#build-inside-a-docker-container) for how
to build the image.

Then run this project inside the container with the sources mounted:

```console
docker run -v ${PWD}:/opt/aos_core_cpp -w /opt/aos_core_cpp -it --rm aos-core-build:latest
```

Inside the container all the commands described above (`./build.sh build`, `test`, `coverage`, etc.) are available.
