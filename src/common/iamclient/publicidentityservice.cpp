...
{
    LOG_INF() << "Init public identity service" << Log::Field("iamPublicServerURL", iamPublicServerURL.c_str())
    ...
    mStub = iamanager::v6::IAMPublicIdentityService::NewStub(
        grpc::CreateCustomChannel(iamPublicServerURL, mCredentials, grpc::ChannelArguments()));
...
Error PublicIdentityService::GetSystemInfo(SystemInfo& info)
{
    ...
    LOG_DBG() << "System info received" << Log::Field("systemID", info.mSystemID.CStr())
              << Log::Field("unitModel", info.mUnitModel.CStr()) << Log::Field("version", info.mVersion.CStr());
...
Error PublicIdentityService::SubscribeListener(aos::iamclient::SubjectsListenerItf& subjectsListener)
{
    ...
    return mSubscriptionManager->AddSubscriber(subjectsListener);
}
...
Error PublicIdentityService::UnsubscribeListener(aos::iamclient::SubjectsListenerItf& subjectsListener)
{
    ...
    if (mSubscriptionManager->RemoveSubscriber(subjectsListener)) {
