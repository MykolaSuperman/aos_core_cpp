...
{
    LOG_INF() << "Init public current node service" << Log::Field("iamPublicServerURL", iamPublicServerURL.c_str())
...
Error PublicCurrentNodeService::GetCurrentNodeInfo(NodeInfo& nodeInfo) const
{
    ...
    LOG_DBG() << "Current node info received" << Log::Field("nodeID", nodeInfo.mNodeID.CStr())
              << Log::Field("nodeType", nodeInfo.mNodeType.CStr());
...
Error PublicCurrentNodeService::SubscribeListener(aos::iamclient::CurrentNodeInfoListenerItf& listener)
{
    ...
    return mSubscriptionManager->AddSubscriber(listener);
}
...
Error PublicCurrentNodeService::UnsubscribeListener(aos::iamclient::CurrentNodeInfoListenerItf& listener)
{
    ...
    if (mSubscriptionManager->RemoveSubscriber(listener)) {
