    ...
    const google::protobuf::Timestamp& timestamp, const String& nodeID, AlertVariant& alertItem)
{
    ...
    alertItem.SetValue(std::move(*alert));
...
Error ConvertFromProto(const servicemanager::v5::ResourceValidateAlert& protoAlert,
    const google::protobuf::Timestamp& timestamp, AlertVariant& alertItem)
{
    (void)protoAlert;
    (void)timestamp;
    ...
    // TODO: Clarify. ResourceValidateAlert is not in the AlertVariant
    return AOS_ERROR_WRAP(Error(ErrorEnum::eNotSupported, "ResourceValidateAlert is not supported"));
}
...
Error ConvertFromProto(
    const servicemanager::v5::EnvVarInstanceStatus& grpcInstanceStatus, EnvVarsInstanceStatus& result)
{
    static_cast<InstanceIdent&>(result) = ConvertToAos(grpcInstanceStatus.instance());

    for (const auto& grpcEnvStatus : grpcInstanceStatus.statuses()) {
        auto err = result.mStatuses.EmplaceBack();
        if (!err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }

        if (auto err = ConvertFromProto(grpcEnvStatus, result.mStatuses.Back()); !err.IsNone()) {
            return err;
        }
    }
...
Error ConvertToProto(const aos::NetworkParameters& src, servicemanager::v5::NetworkParameters& dst)
{
    dst.set_network_id(src.mNetworkID.CStr());
    dst.set_subnet(src.mSubnet.CStr());
    dst.set_ip(src.mIP.CStr());

    // TODO: Clarify. vlan_id is not available in NetworkParameters proto.
...
Error ConvertToProto(const aos::InstanceInfo& src, servicemanager::v5::InstanceInfo& dst)
{
    ...
    *instance      = pbconvert::ConvertToProto(static_cast<const InstanceIdent&>(src));
    ...
    auto* networkParams = dst.mutable_network_parameters();
    if (auto err = ConvertToProto(src.mNetworkParameters, *networkParams); !err.IsNone()) {
        return err;
    }
...
Error ConvertToProto(const NodeConfig& config, servicemanager::v5::CheckNodeConfig& result)
{
    ...
    // TODO: Not clear how to convert NodeConfig to JSON string
...
Error ConvertFromProto(const servicemanager::v5::Alert& grpcAlert, const String& nodeID, AlertVariant& alertItem)
{
    ...
    } else if (grpcAlert.has_resource_validate_alert()) {
        return ConvertFromProto(grpcAlert.resource_validate_alert(), grpcAlert.timestamp(), alertItem);
...
{
    result.set_log_id(log.mLogID.CStr());
...
{
    if (auto err = aosPushLog.mLogID.Assign(grpcLogData.log_id().c_str()); !err.IsNone()) {
        return AOS_ERROR_WRAP(err);
    }
...
Error ConvertToProto(const OverrideEnvVarsRequest& envVars, servicemanager::v5::OverrideEnvVars& result)
{
    for (const auto& item : envVars.mItems) {
        auto* envVarItem = result.add_env_vars();

        auto* filter = envVarItem->mutable_instance_filter();
        if (item.mItemID.HasValue()) {
            filter->set_item_id(item.mItemID.GetValue().CStr());
        }
        if (item.mSubjectID.HasValue()) {
            filter->set_subject_id(item.mSubjectID.GetValue().CStr());
        }
        if (item.mInstance.HasValue()) {
            filter->set_instance(item.mInstance.GetValue());
        }

        for (const auto& var : item.mVariables) {
            auto* envVar = envVarItem->add_variables();

            envVar->set_name(var.mName.CStr());
            envVar->set_value(var.mValue.CStr());

            if (var.mTTL.HasValue()) {
                *envVar->mutable_ttl() = TimestampToPB(var.mTTL.GetValue());
            }
        }
    }
...
{
    for (const auto& grpcEnvVarStatus : grpcStatus.instances()) {
        auto err = result.mStatuses.EmplaceBack();
        if (!err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }

        if (auto err = ConvertFromProto(grpcEnvVarStatus, result.mStatuses.Back()); !err.IsNone()) {
            return err;
        }
    }
...
{
    dst.set_image_id(src.mImageID.CStr());
    dst.set_version(src.mVersion.CStr());
    dst.set_url(src.mURL.CStr());
    dst.set_sha256(reinterpret_cast<const char*>(src.mSHA256.Get()), src.mSHA256.Size());
    dst.set_size(static_cast<uint64_t>(src.mSize));
    ...
    const servicemanager::v5::AverageMonitoring& src, const String& nodeID, aos::monitoring::NodeMonitoringData& dst)
{
    ...
    for (const auto& inst : src.instances_monitoring()) {
        if (auto err = dst.mServiceInstances.EmplaceBack(); !err.IsNone()) {
            return err;
        }

        if (auto err = ConvertFromProto(inst, dst.mServiceInstances.Back()); !err.IsNone()) {
            return err;
        }
    }
...
Error ConvertFromProto(const servicemanager::v5::InstanceStatus& src, const String& nodeID, aos::InstanceStatus& dst)
{
    ...
    // TODO: clarify. state checksum is missing in prot
